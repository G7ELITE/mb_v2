...
def get_redis_adapter() -> RedisAdapter:
    ...
    try:
        ...
        if not settings.REDIS_URL:
            logger.info({"evt": "redis_fallback", "mode": "inmemory", "reason": "no_url"})
        ...
        try:
            ...
            if adapter.ping():
                ...
                logger.info({"evt": "redis_ping_failed", "mode": "inmemory"})
                return InMemoryRedis()
        except Exception as e:
            logger.warning(f"Redis não disponível: {e}, usando in-memory")
            logger.info({"evt": "redis_fallback", "mode": "inmemory", "reason": str(e)})
            return InMemoryRedis()
            
    except Exception as e:
        # Fallback absoluto se até settings falhar
        logger.error(f"Erro crítico no Redis adapter: {e}")
        logger.info({"evt": "redis_fallback", "mode": "inmemory", "reason": "critical_error"})
        return InMemoryRedis()
